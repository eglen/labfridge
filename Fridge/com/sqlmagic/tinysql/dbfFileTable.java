 ...
 */
      ...
      if ( debug ) 
         System.out.println("dbfFileTable: fileName=" + fileName + " table="
         + table + " fullPath=" + fullPath);
/*
 *    Open the DBF file
 */
 ...
 */
   public void close() throws tinySQLException 
   {
      try 
      {
         if ( debug ) System.out.println("Closing " + toString());
         ftbl.close();
         fileOpen = false;
      } catch (IOException e) {
 ...
 */
         tsColumn column = (tsColumn) column_info.get(colName);
         if (column == null)
 ...
 */
   Hashtable open_dbf() throws tinySQLException 
   {
      try
      {
         ...
         if (debug) System.out.println("Try to open  " + f.getAbsolutePath());
         if (!f.exists() ) 
         {
